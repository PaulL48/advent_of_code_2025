use std::{cmp::Ordering, collections::HashSet, ops::Sub};

use get_input::get_input;

fn main() {
    let input = get_input(8).unwrap().lines().map(|l| Vec3::from_str(l)).collect::<Vec<_>>();
    part1(&input, 1000);
    part2();
}

fn part1(input: &[Vec3], take: usize) {
    // First we try the naive approach
    ...
    for (_distance, j1, j2) in distances.iter().take(take) {
        // print!("Checking {:?} <-> {:?} ", j1, j2);
        // Check that if one is a circuit that they both aren't in the same circuit
        if !get_circuit(&mut circuits, j1).is_none() && get_circuit(&mut circuits, j1) == get_circuit(&mut circuits, j2) {
    ...
    println!("Part 1: Product of three largest circuit sizes {}", circuit_sizes.iter().cloned().take(3).product::<u64>());
}
...
fn f32_sort(l: &f32, r: &f32) -> Ordering {
    l.total_cmp(r)
}

fn get_circuit<'a>(circuits: &'a mut Vec<HashSet<Vec3>>, j: &Vec3) -> Option<usize> {
...
fn merge_junctions(circuits: &mut Vec<HashSet<Vec3>>, j1: &Vec3, j2: &Vec3) {
    ...
    match (j1_circuit, j2_circuit) {
        ...
        (None, Some(c)) => { circuits[c].insert(*j1); },
        (Some(c), None) => { circuits[c].insert(*j2); },
        (None, None) => {
